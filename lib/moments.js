...
function normalizeMoment(data) {
  ...
  return {
    ...
    // Đánh dấu "yêu thích" - dữ liệu cũ (trước khi có tính năng này) sẽ không
    // có field này nên mặc định coi là false.
...
// Đánh dấu / bỏ đánh dấu "yêu thích" cho 1 album (dùng cho bộ lọc "Yêu thích"
// ở trang Album trong giao diện mới).
export async function setMomentFavorite(id, favorite) {
  const ref = doc(db, COLLECTION, id);
  await updateDoc(ref, { favorite: !!favorite });
}